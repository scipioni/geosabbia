...
class Sabbiera: public Scene {
private:
	GLuint texture_sabbia;
public:
	Sabbiera(const char* name);
	void KeyDown(int key);
	void Initialize();
	void Update();
	void Draw(int pass);
	void Cannone(bool mViscous);
	void Pioggia();
	void Bordi();
	void Blocchi();
};
...
public:
	...
	void Initialize()
	{
		float radius = 0.05f;
		// 				z 
		CreateParticleGrid(Vec3(0.4f, 4.0f + radius*0.5f, 0.1f), 16, 4, 16, radius, Vec3(0.0f), 1.0f, false, 0.0f, flexMakePhase(0, eFlexPhaseSelfCollide), 0.0f);

		g_staticMesh = CreateDiscMesh(1.0f, 8);
		g_staticMesh->Transform(TranslationMatrix(Point3(0.0f, 1.5f, 0.0f)));
		g_staticMesh->m_positions[0].y -= 0.75f;	
...
public:
	...
	virtual void Initialize()
	{
		...
		g_params.mNumPlanes = 5;		DrawMesh(g_staticMesh, g_meshColor);
