...
#define MEMORY_TYPE eFlexMemoryHost
#define MEMORY_TYPE_ASYNC eFlexMemoryDeviceAsync
...
{	
	...
	flexSetParticles(g_flex, (float*)&g_positions[0], numParticles, MEMORY_TYPE);
	flexSetVelocities(g_flex, (float*)&g_velocities[0], numParticles, MEMORY_TYPE);
	flexSetNormals(g_flex, (float*)&g_normals[0], numParticles, MEMORY_TYPE);
	
	g_activeIndices.resize(maxParticles);
	for (size_t i=0; i < g_activeIndices.size(); ++i)
		g_activeIndices[i] = i;

	flexSetActive(g_flex, &g_activeIndices[0], numParticles, MEMORY_TYPE);
	...
	if (g_convexPositions.size())
	{
		flexSetConvexes(
			...
			&g_convexFlags[0],
			g_convexStarts.size(),
			g_convexPlanes.size(), MEMORY_TYPE);
	}
	...
	if (g_rigidOffsets.size())
	{
		...
		flexSetRigids(g_flex, &g_rigidOffsets[0], &g_rigidIndices[0], (float*)&g_rigidLocalPositions[0], g_rigidLocalNormals.size()?(float*)&g_rigidLocalNormals[0]:NULL, &g_rigidCoefficients[0], (float*)&g_rigidRotations[0], numRigids, MEMORY_TYPE);
	}
	...
	if (g_springIndices.size())
	{
		...
		flexSetSprings(g_flex, &g_springIndices[0], &g_springLengths[0], &g_springStiffness[0], g_springLengths.size(), MEMORY_TYPE);
	}

	if (g_triangles.size())
	{
		flexSetDynamicTriangles(g_flex, &g_triangles[0], &g_triangleNormals[0].x, g_triangles.size()/3, MEMORY_TYPE);
	}
	...
	if (g_staticMesh)
	{
		flexSetTriangles(g_flex, (int*)&g_staticMesh->m_indices[0], (float*)&g_staticMesh->m_positions[0], g_staticMesh->GetNumFaces(), g_staticMesh->GetNumVertices(), g_params.mRadius*4.0f, MEMORY_TYPE);
	}


	flexSetPhases(g_flex, &g_phases[0], g_phases.size(), MEMORY_TYPE);
	...
	if (g_warmup)
	{
		...
		for (int i=0; i < kWarmupIterations; ++i)
		{
			...
			flexSetVelocities(g_flex, (float*)&g_velocities[0], maxParticles, MEMORY_TYPE);
		}

		// udpate host copy
		flexGetParticles(g_flex, &g_positions[0].x, g_positions.size(), MEMORY_TYPE);
...
void GLUTUpdate()
{
	...
	{	
		...
		{			
			...
			flexSetActive(g_flex, &g_activeIndices[0], activeCount, MEMORY_TYPE);
		}
		...
		flexSetParticles(g_flex, &g_positions[0].x, g_positions.size(), MEMORY_TYPE);
		flexSetVelocities(g_flex, &g_velocities[0].x, g_velocities.size(), MEMORY_TYPE);
		flexSetPhases(g_flex, &g_phases[0], g_phases.size(), MEMORY_TYPE);
	...
	flexGetParticles(g_flex, &g_positions[0].x, g_positions.size(), MEMORY_TYPE);
	flexGetVelocities(g_flex, &g_velocities[0].x, g_velocities.size(), MEMORY_TYPE);
	flexGetNormals(g_flex, &g_normals[0].x, g_normals.size(), MEMORY_TYPE);
	
	// readback normals
	if (g_triangles.size())
		flexGetDynamicTriangles(g_flex, &g_triangles[0], &g_triangleNormals[0].x, g_triangles.size()/3, MEMORY_TYPE);
	...
	if (g_drawEllipsoids)
		flexGetSmoothParticles(g_flex, (float*)positionsPtr, numParticles, MEMORY_TYPE_ASYNC);
	else
		flexGetParticles(g_flex, (float*)positionsPtr, numParticles, MEMORY_TYPE_ASYNC);

	flexGetAnisotropy(g_flex, (float*)anisotropyPtr[0], (float*)anisotropyPtr[1], (float*)anisotropyPtr[2], MEMORY_TYPE_ASYNC);
	
	if (g_drawDensity)
		flexGetDensities(g_flex, (float*)densityPtr, MEMORY_TYPE_ASYNC);
	else
		flexGetPhases(g_flex, (int*)densityPtr, numParticles, MEMORY_TYPE_ASYNC);
	...
	if (g_renderBuffers.mNumDiffuseParticles)
	{
		...
		numDiffuse = flexGetDiffuseParticles(g_flex, (float*)diffusePosPtr,(float*)diffuseVelPtr, (int*)diffuseIdxPtr, MEMORY_TYPE_ASYNC);
	}
	...
	{
		flexGetRigidTransforms(g_flex, (float*)&g_rigidRotations[0], (float*)&g_rigidTranslations[0], MEMORY_TYPE);
		SkinMesh();
	}
	...
	if (g_drawNormals)
	{
		...
		flexGetNormals(g_flex, (float*)&g_normals[0], g_normals.size(), MEMORY_TYPE);
	...
	switch (key)
	{
		...
		case 'c':
		{
			...
			if (!g_ffmpeg)
			{
				...
				do
				{
					sprintf(buf, "/tmp/output%d.mp4", i);
					f = fopen(buf, "rb");
					if (f)
						fclose(f);
		...
		case GLUT_DOWN:
		{
			...
			if (glutGetModifiers() == GLUT_ACTIVE_SHIFT && lastb == GLUT_LEFT_BUTTON)
			{
				...
				if (0 && g_mouseParticle != -1)
				{
					...
					// add particle to sim
					int numActive = flexGetActiveCount(g_flex);
					flexSetActive(g_flex, &g_activeIndices[0], numActive+1, MEMORY_TYPE);

					flexSetParticles(g_flex, (float*)&g_positions[0], g_positions.size(), MEMORY_TYPE);
					flexSetVelocities(g_flex, (float*)&g_velocities[0], g_velocities.size(), MEMORY_TYPE);
					flexSetPhases(g_flex, &g_phases[0], g_phases.size(), MEMORY_TYPE);
					flexSetSprings(g_flex, &g_springIndices[0], &g_springLengths[0], &g_springStiffness[0], g_springLengths.size(), MEMORY_TYPE);
				}
			}
...
{		
	...
	g_scenes.push_back(new Sabbiera("Sabbiera"));
	...
	if (!fullscreen)
	{
		glutCreateWindow("Sabbia interattiva");
	...
	{https://youtu.be/vYDsEzKDhbAhttps://youtu.be/vYDsEzKDhbA
...
Sabbiera::Sabbiera(const char* name) :
		Scene(name) {
}

void Sabbiera::KeyDown(int key) {
	printf("key %d  g_camPos.x=%f g_camPos.y=%f g_camPos.z=%f\n", key, g_camPos.x, g_camPos.y, g_camPos.z);
	switch (key) {
	case 'G': {
		sabbia->up(0.01f);
		break;
	...
	case 'O': {
			sabbia->h_delta = 0.01*sabbia->h_delta*g_params.mRadius/abs(sabbia->h_delta);
			sabbia->ondula();
			break;
		}
	case '0': {
	...
	case 'N': {
			sabbia->toggle_normals();
			break;
		}
	}
}

void Sabbiera::Update() {
	sabbia->UpdateCollisions(g_flex, g_params.mRadius);
}


void Sabbiera::Draw(int pass) {
    //if (pass==0)
	sabbia->draw();
}

void Sabbiera::Cannone(bool mViscous = true) {
	float radius = 0.08f;
...
void Sabbiera::Pioggia() {
	...
	CreateParticleGrid(Vec3(5.0f, 5.0f + radius * 0.5f, 5.0f), 16, 40, 16,
			radius, Vec3(0.0f), 1.0f, false, 0.0f,
			flexMakePhase(0, eFlexPhaseSelfCollide | eFlexPhaseFluid), 0.0f);
...
void Sabbiera::Bordi() {
	...
	Vec3 center = Vec3(width, 0.0, width);
	CreateConvex(Vec3(edge, height, width), center + Vec3(-width-edge, height/2.0f, 0.0f));
	CreateConvex(Vec3(edge, height, width), center + Vec3(width+edge, height/2.0f, 0.0f));

	CreateConvex(Vec3(width+2*edge, height, edge), center + Vec3(0.0f, height/2.0f, width+edge));
	CreateConvex(Vec3(width+2*edge, height, edge), center + Vec3(0, height/2.0f, -width-edge));

}

void Sabbiera::Blocchi() {
	float minSize = 0.5f;
	float maxSize = 1.0f;
	for (int i = 0; i < 5; i++)
		CreateRandomConvex(10, Vec3(i * 2.0f, 0.0f, Randf(0.0f, 2.0f)), minSize,
				maxSize, Vec3(0.0f, 1.0f, 0.0f), Randf(0.0f, k2Pi * 10.0f));
}

void Sabbiera::Initialize() {
	g_numSubsteps = 2;
	g_drawDensity = true;
	g_drawDiffuse = true;
	...
	/* camera */
	g_sceneLower = Vec3(0.0f, 0.0f, -0.5f); // x y z
	g_sceneUpper = Vec3(20.0f, 0.0f, -0.5f);
	g_camPos.x = 4.77f;
	g_camPos.y = 6.22f;
	g_camPos.z = 11.8f;
	g_camAngle = Vec3(0.0f, -DegToRad(45.0f), 0.0f);

	g_params.mNumPlanes = 0; // togliamo il piano

	sabbia = new Sabbia(101, false); // solo dispari
	sabbia->ondula(1.0);

	//int w,h;
	//texture_sabbia = png_texture_load("data/sabbia.png", &w, &h);
}
